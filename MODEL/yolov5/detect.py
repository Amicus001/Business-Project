...
from de_identification import Deident
...
def run(
    ...
    mosaic_type=0,  # face mosaic type
):
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # det_for_sort = det[:, :5].cpu().numpy()

                # Update SORT tracker
                # tracked_objects = tracker.update(det_for_sort)

                # Draw bounding boxes and labels of tracked objects
                # for *xyxy, obj_id in tracked_objects:
                #     # c = int(det[0, 5])  # Assuming all detections are of the same class
                #     # label = f"ID {int(obj_id)} {names[c]}"
                #     # annotator.box_label(xyxy)
                #     annotator.apply_ellipse_blur(xyxy)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if mosaic_type == 1:
                            deident.apply_rectangle_mosaic(xyxy)
                        elif mosaic_type == 2:
                            deident.apply_ellipse_mosaic(xyxy)
                        elif mosaic_type == 3:
                            deident.apply_rectangle_blur(xyxy)
                        elif mosaic_type == 4:
                            deident.apply_ellipse_blur(xyxy)
...
def parse_opt():
    ...
    parser.add_argument("--mosaic-type", type=int, default=0, help="face mosaic type")
